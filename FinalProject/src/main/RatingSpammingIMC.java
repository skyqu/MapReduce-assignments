...
public class RatingSpammingIMC extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(RatingSpammingIMC.class);

  /*
   * Mapper1 reads records from the review dataset and emits <(member id, product id), rating> as
   * key-value pairs.
   */
  private static class MyMapper1 extends Mapper<LongWritable, Text, PairOfStrings, PairOfInts> {
    private final static PairOfInts Rate = new PairOfInts();
    private final static PairOfStrings Pair = new PairOfStrings();
    private static Hashtable<String,Integer> htavg = new Hashtable<String,Integer>();
    private static Hashtable<String,Integer> htavgcounter=new Hashtable<String,Integer>();
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException,
      ...
      if (terms.length > 5) {
        ...
        int rate = Integer.parseInt(terms[5].trim().substring(0, 1));
   	  	if(!htavg.containsKey(pid)&&pid.length()!=0)  
   	  	{
		  htavg.put(pid, rate);
		  htavgcounter.put(pid, 1);
   	  	}
   	  	else if(pid.length()!=0){
		  htavg.put(pid,(Integer)htavg.get(pid)+rate);
		  htavgcounter.put(pid,(Integer)htavgcounter.get(pid)+1);
   	  	}
        Rate.set(rate,1);
        Pair.set(pid, uid);
    ...
    public void cleanup(Context context)
    	...
    	//combine the DF in mapper and then emit;
        	for(Enumeration<String> e = htavg.keys();e.hasMoreElements();){
        		String fkey=e.nextElement();
        		Pair.set(fkey,"\0");
        		Rate.set((Integer)htavg.get(fkey),(Integer)htavgcounter.get(fkey));
        		context.write(Pair,Rate);   
           }
        }
  ...
  protected static class MyPartitioner extends Partitioner<PairOfStrings, PairOfInts> {
    @Override
  ...
  private static class MyReducer1 extends
      Reducer<PairOfStrings, PairOfInts, PairOfStrings, PairOfFloats> {
    ...
    public void reduce(PairOfStrings key, Iterable<PairOfInts> values, Context context)
      ...
      if (key.getRightElement().equals("\0")) {
        ...
        while (iter.hasNext()) {
          PairOfInts temp = iter.next().clone();
          avgRate += temp.getLeftElement();
          count += temp.getRightElement();
        }
        avgRate = avgRate / count;
      } else {
        while (iter.hasNext()) {
          List.add(iter.next().getLeftElement());
        }
        if (List.size() > 1) {
          int similarity = 0;
          float rateDev = 0;
  ...
  public int run(String[] args) throws Exception {
    ...
    String outputPath ="xzzqskfinal/RSScoreIMC";         //cmdline.getOptionValue(OUTPUT);
    int reduceTasks = cmdline.hasOption(NUM_REDUCERS) ? Integer.parseInt(cmdline
        .getOptionValue(NUM_REDUCERS)) : 1;

    LOG.info("Tool: " + RatingSpammingIMC.class.getSimpleName());
    ...
    job1.setJobName(RatingSpammingIMC.class.getSimpleName());
    job1.setJarByClass(RatingSpammingIMC.class);
    ...
    job2.setJobName(RatingSpammingIMC.class.getSimpleName());
    job2.setJarByClass(RatingSpammingIMC.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new RatingSpammingIMC(), args);
  }
}