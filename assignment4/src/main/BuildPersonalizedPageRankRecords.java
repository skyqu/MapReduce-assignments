...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNodeMultiSrc> {
    ...
    @Override
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNodeMultiSrc>.Context context) {
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      for(int i=0;i<src.length;i++){
    	  if (nid.get() == src[i])
    	  {
    		  node.setSrcNodePG(src[i], 1.0f);
    		  context.write(nid, node);
    		  node.setSrcNodePG(src[i], 0.0f);
    		  return;
    	  }
    	  else
    	  {
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("src").hasArg()
            .withDescription("source of pagerank").create(SOURCES));
