...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static final String NODE_SRC="source_pagerank";
  ...
  private static class MapClass extends
      Mapper<IntWritable, PageRankNodeMultiSrc, IntWritable, PageRankNodeMultiSrc> {
    ...
    private static final PageRankNodeMultiSrc intermediateMass = new PageRankNodeMultiSrc();

    // For passing along node structure.
    private static final PageRankNodeMultiSrc intermediateStructure = new PageRankNodeMultiSrc();
    
    private static final HMapIFW InterMass = new HMapIFW();

    @Override
    public void map(IntWritable nid, PageRankNodeMultiSrc node, Context context)
        throws IOException, InterruptedException {
      // Pass along node structure.
      intermediateStructure.setNodeId(node.getNodeId());
      intermediateStructure.setType(PageRankNodeMultiSrc.Type.Structure);
      ...
      if (node.getAdjacenyList().size() > 0) {
        ...
        intermediateMass.setType(PageRankNodeMultiSrc.Type.Mass);
        ArrayListOfIntsWritable list = node.getAdjacenyList();
        int degree = list.size();
        //System.out.print(PGlist[0]+"\n\n"+"HIHIHIHIHIHIHHIIHIH");
        for(int j=0;j<PGlist.length;j++)
        {
        	InterMass.put(PGlist[j], 0);
        }
        for(int i=0;i<PGlist.length;i++)
        {
        	//random jump to source node
        	InterMass.put(PGlist[i], (ALPHA)*node.getPageRank(PGlist[i]));
            intermediateMass.setNodeId(PGlist[i]);
            intermediateMass.setPageRank(0);
            intermediateMass.setPageRank(InterMass);
            neighbor.set(PGlist[i]);
            context.write(neighbor, intermediateMass);
            massMessages++;
        	InterMass.put(PGlist[i], 0);
        }
        
        for(int i=0;i<PGlist.length;i++)
        {
        	InterMass.put(PGlist[i], ((1-ALPHA)*node.getPageRank(PGlist[i]))/degree);
        }
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          context.write(neighbor, intermediateMass);
          if(intermediateMass.getPageRank(9627181)>=0.0255&&intermediateMass.getPageRank(9627181)<=0.026)
          {
         	  System.out.print(nid.get()+"  "+neighbor.toString()+intermediateMass.toString()+"\n\n");
         }
          massMessages++;
      ...
      //Dangling node emit PageRank back to Source node
      else{
          intermediateMass.setType(PageRankNodeMultiSrc.Type.Mass);
          intermediateMass.setPageRank(0);
          ...
          for(int i=0;i<PGlist.length;i++)
          {
          	InterMass.put(PGlist[i], node.getPageRank(PGlist[i]));
            intermediateMass.setNodeId(PGlist[i]);
            intermediateMass.setPageRank(InterMass);
            neighbor.set(PGlist[i]);
            context.write(neighbor, intermediateMass);
  ...
  private static class CombineClass extends
      Reducer<IntWritable, PageRankNodeMultiSrc, IntWritable, PageRankNodeMultiSrc> {
    private static final PageRankNodeMultiSrc intermediateMass = new PageRankNodeMultiSrc();

    @Override
    public void reduce(IntWritable nid, Iterable<PageRankNodeMultiSrc> values, Context context)
        throws IOException, InterruptedException {
      int massMessages = 0;

      // Remember, PageRank mass is stored as a log prob.
      //float mass = Float.NEGATIVE_INFINITY;
      for (PageRankNodeMultiSrc n : values) {
        if (n.getType() == PageRankNodeMultiSrc.Type.Structure) {
          // Simply pass along node structure.
          context.write(nid, n);
        } else {
          // Accumulate PageRank mass contributions.
          intermediateMass.Sum(n.getPageRankM());
          massMessages++;
        }
      }
  ...
  private static class ReduceClass extends
    ...
    private PageRankNodeMultiSrc totalMass = new PageRankNodeMultiSrc();
    private static String SrcNode;
    private static int[] src ;
    private static PageRankNodeMultiSrc node = new PageRankNodeMultiSrc();
    @Override
    public void setup(Reducer<IntWritable, PageRankNodeMultiSrc, IntWritable, PageRankNodeMultiSrc>.Context context) {
        
    	SrcNode = context.getConfiguration().get(NODE_SRC);

        String[] sources = SrcNode.split(",");
        node.setType(PageRankNodeMultiSrc.Type.Complete);
        node.setPageRank(0);   //unused
        //System.out.print(sources[0]+sources[1]+"\n\n\n\n");
        src =new int[sources.length];
        for (int i = 0; i < sources.length; i++) {
            src[i] = Integer.parseInt(sources[i]);
          }
    ...
    @Override
    public void reduce(IntWritable nid, Iterable<PageRankNodeMultiSrc> iterable, Context context)
        throws IOException, InterruptedException {
      Iterator<PageRankNodeMultiSrc> values = iterable.iterator();
      ...
      while (values.hasNext()) {
        PageRankNodeMultiSrc n = values.next();

        if (n.getType().equals(PageRankNodeMultiSrc.Type.Structure)) {
      ...
      if (structureReceived == 1) {
        ...
        totalMass.Sum(node.getPageRankM());
    ...
    public void cleanup(Context context) throws IOException {
     ...
     /*FileSystem fs = FileSystem.get(context.getConfiguration());
      ...
      int[] key=totalMass.getSrc();
      for(int i=0; i<key.length;i++)
      {
    	  out.writeFloat(totalMass.getPageRank(key[i]));
      }
	  out.close();*/
    }
  }
 ...
 /* private static class MapPageRankMassDistributionClass extends
  ...
  public int run(String[] args) throws Exception {
    ...
    options.addOption(OptionBuilder.withArgName("sources").hasArg()
            .withDescription("personalized source nodes").create(SRC_NODES));
    ...
    if (!cmdline.hasOption(BASE) || !cmdline.hasOption(START) ||
      ...
      return -1;
		...
		String src = cmdline.getOptionValue(SRC_NODES);
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
    ...
    phase1(i, j, basePath, numNodes, useCombiner, useInMapperCombiner);
